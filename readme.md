...
>Echo 是一个以聊天为主入口的可观察 LLM workspace。它不是“固定模板式 RAG”，而是一条由 LangGraph 控制的决策 workflow：模型先 `plan`，按需 `retrieve`，统一通过 `tool` 执行动作，再 `think` 是否继续，最后输出 `answer`。
...
- 检索工具：`mcp_server/tools/`
- skills 文档：`echo/skills/`
...
├── mcp_server/
│   ├── rag/                  # vector DB + embedding client integration
